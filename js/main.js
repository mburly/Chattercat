...
function loadChannelPage(id)
{
    document.body.scrollTop = document.documentElement.scrollTop = 0;
    var channel = id.split("-")[0];
    $.post("php/stats.php", {channel: channel})
        .done(function(data) {
            data = JSON.parse(data);
            hide("main");
            document.title = channel + " - Chattercat"
            generateChannelPage(channel);
            $.post("php/updates.php", {channel: channel})
...
function msToTime(ms) {
    return `${Math.floor(ms / 86400000) > 0 ? Math.floor(ms / 86400000) + 'd' : ''} ${Math.floor((ms % 86400000) / 3600000) > 0 ? Math.floor((ms % 86400000) / 3600000) +'h' : '' } ${Math.floor((ms % 3600000) / 60000)}m`;
}
